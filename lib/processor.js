...
module.exports = {
    ...
    processStoredIds : function () {
        ...
        // batch up the usere ids we have in the store
        for (var i = 0; i < store.length; i=i+10) {
            // check if we have filled up the max filter size
            if(Buffer.byteLength(processIds.toString(), 'utf8') <= 35 && i >= 10){
                var currentIds = store.splice(0, i);
                processIds.push(currentIds);
            }
        }

        console.log(this.getCsdl('['+processIds+']'));
        return true;
    },
