...
module.exports = {
    ...
    process : function (name, data) {
        ...
        if (Object.prototype.toString.call(data) === '[object Object]') {
            ...
            if(store.length > 0 && currentIds !== false){
    ...
    processStoredIds : function () {
        ...
        for (var i = 0; i < store.length; i=i+100) {
            ...
            if(Buffer.byteLength(processIds.toString(), 'utf8') <= 734003 && i >= 1){
                var currentIds = store.splice(0, i);
                processIds = processIds.concat(currentIds);
            }
            if(Buffer.byteLength(processIds.toString(), 'utf8') > 734003 /*|| store.length === 0*/){
