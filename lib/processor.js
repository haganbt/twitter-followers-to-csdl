...
module.exports = {
    ...
    process : function (name, data) {
        ...
        if (Object.prototype.toString.call(data) === '[object Object]') {
            ...
            if (currentIds !== false) {
                logger.debug('Building CSDL and compiling filter.');
                var self = this;
                this.getStreamHash(this.getCsdl('[' + currentIds + ']'), function (err, res) {
                    ...
                    logger.debug('Saving new hash: ' + dataSiftResponse.hash);
                    self.setHash(dataSiftResponse.hash);
                    utils.saveFile(name + '.csdl', "\n\nSaved at " + moment().format('MMMM Do YYYY, h:mm:ss a') + ":\n" + self.getHashString());
                    logger.debug('Final CSDL: ', self.getHashString());
