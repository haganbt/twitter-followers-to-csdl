...
module.exports = {
     ...
     *
     * CSDL Size Limit: Each filter has a limit of around 0.7Mb however setting
     * the limit to around 0.66 Mb means that a typical rate limit user of Twitter
     * will get a filter compiled on each set of requests. At the time of writing
     * Twitter supports 15 requets, each returning 5k user ids = 75000 every 15
     * minutes.
     *
    ...
    processStoredIds : function () {
        ...
        if(this.getStoreCount() === 0 || Buffer.byteLength(store.toString(), 'utf8') < 700000){
        ...
        for (var i = 0; i < store.length; i=i+100) {
            ...
            if(Buffer.byteLength(processingIds.toString(), 'utf8') >= 700000){
